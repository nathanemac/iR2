...
my_res = MPR2(nlp, h, options, activate_mp=true) # launches vanilla R2-Reg (one might add verbose=1 for more verbosity)
...
p1 = performance_profile(stats, df->Float64.(df.neval_grad))
ylabel!("")
p2 = performance_profile(stats, df->Float64.(df.elapsed_time))
plot!(title="elapsed_time")
