...
function test_condition_f(fk, sk, σk, κf, pf, ps, Π, k, verb) # p : current level of precision
  while abs(fk[pf])*(1- 1/(1 + eps(Π[pf]))) > κf*σk*norm(sk[ps])^2
    if ((Π[pf] == Π[end]) && (Π[ps] == Π[end]))
      verb ==true && @warn "maximum precision already reached on f and s."
      break
    end
...
function test_condition_h(hk, sk, σk, κh, ph, ps, Π, k, verb) # p : current level of precision
  while abs(hk[ph])*(1- 1/(1 + eps(Π[ph]))) > κh*σk*norm(sk[ps])^2
    ((Π[ph] == Π[end]) && (Π[ps] == Π[end])) && verb ==true && @warn "maximum precision already reached on h and s."
...
function test_condition_∇f(gfk, sk, σk, κ∇, pg, ps, Π, k, verb) # p : current level of precision
  while norm(gfk[pg])*(1- 1/(1 + eps(Π[pg]))) > κ∇*σk*norm(sk[ps])
    ((Π[pg] == Π[end]) && (Π[ps] == Π[end])) && verb ==true && @warn "maximum precision already reached on ∇f and s."
...
# check assumption 6
function test_assumption_6(ξ, κs, σk, sk, Π, ps, ph, mk, hk, k, verb)
  while ξ < 1/2*κs*σk*norm(sk[ps])^2
    if (Π[ps] == Π[end]) && (Π[ph] == Π[end])
      verb ==true && @warn "maximum precision already reached on h and s to satisfy Assumption 6."
    end
