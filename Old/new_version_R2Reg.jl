mutable struct R2Solver{R<:Real, S<:AbstractVector, G <: Union{ShiftedProximableFunction, Nothing}} <: AbstractOptimizationSolver
  xk::Vector{S}
  mν∇fk::Vector{S}
  gfk::Vector{S}
  fk::S
...
The algorithm stops either when `√(ξₖ/νₖ) < atol + rtol*√(ξ₀/ν₀) ` or `ξₖ < 0` and `√(-ξₖ/νₖ) < neg_tol` where ξₖ := f(xₖ) + h(xₖ) - φ(sₖ; xₖ) - ψ(sₖ; xₖ), and √(ξₖ/νₖ) is stationarity measure.
